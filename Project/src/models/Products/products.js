...
function Products()
{
    ...
    useEffect(()=>
    {
        ...
        for(var i=0; i<temp.length; i++)
        {
            ...
            {
                let prod= purchases.filter(x=> x.ProductID==temp1[j] && x.CustomerID==temp[i]);
                let date= purchases.filter(x=> x.ProductID==temp1[j] && x.CustomerID==temp[i]).map(x=> x.Date);
    ...
    return(<div className='size'>
        ...
        <div style={{float: "left", width: "43%"}}>
            ...
            <Link to="addProduct"><input type="button" value="Add Product" className='button'/></Link><br/><br/>
          
            {
                storeData[0][1].map((item,index)=>
                {
                    return <div key={index}><div className='border' style={{width: "85%"}}>

                        <b>Product Name: </b> &nbsp;&nbsp;<Link to={"editProduct/"+ item.ID}>{item.Name}</Link><br/>
                        <b>Product Price: </b> &nbsp;&nbsp;{item.Price}$<br/>
                        <b>Product Quantity: </b> &nbsp;&nbsp;{item.Quantity}<br/><br/>
                            ...
                            <>{
                                all.map((x,i)=>
                                {
                                    return <div key={i}>
                                        ...
                                        {x.Products.find(z=> z.ProductID==item.ID)!= undefined ? 
                                        
                                            <>  
                                                <ul>
                                                    <li><Link to={"/customers/editCustomer/"+ x.CustomerID}>{customers.find(z=> z.ID==x.CustomerID).Fname} {customers.find(z=> z.ID==x.CustomerID).Lname} </Link></li>
                                                    
                                                    <li>
                                                    {
                                                        x.Products.map((y,j)=>
                                                        {
                                                            return <div key={j}>{y.ProductID==item.ID ?
                                                ...
                                                <input type="button" value="Add" onClick={()=> (setAdd(true),setId(x.CustomerID))} className='button'/><br/>
        ...
        {add==true ?
            ...
            <div style={{float: "left", width: "30%", textAlign: "left"}}>
                ...
                <h3> Customer: <u>{customers.find(z=> z.ID==id).Fname} {customers.find(z=> z.ID==id).Lname} </u> </h3>
                ...
                <input type="button" value="Buy" onClick={addProducts} className='button'/><br/>
