...
function reducer(state = [] ,action) 
{
    switch(action.type)
    {
        ...
        case "addPurchase":
        {
            ...
            let arr= state[0][2], temp

            for(var i=0; i<action.payload[1].length; i++)
            {
                if(arr.length>0)
                temp={ID: arr[arr.length-1].ID +1, CustomerID: action.payload[0], ProductID: action.payload[1][i], Date: date};

                else
                temp={ID: 1, CustomerID: action.payload[0], ProductID: action.payload[1][i], Date: date};  
